...
void common_chat_grammar_to_sampler(const common_chat_params * src, const llama_vocab * vocab,
    ...
    for (const auto & trigger : src->grammar_triggers) {
        if (trigger.type == COMMON_GRAMMAR_TRIGGER_TYPE_WORD) {
            ...
            if (ids.size() == 1) {
                auto token = ids[0];
                auto found = std::find(dst.preserved_tokens.begin(), dst.preserved_tokens.end(), (llama_token) token);
