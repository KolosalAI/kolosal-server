...
namespace kolosal {
    ...
    bool DownloadManager::cancelDownload(const std::string& model_id) {
        ...
        if (it != downloads_.end() && it->second->status == "downloading") {
            ...
            // Note: CURL download cancellation would require additional implementation
            // For now, we just mark it as cancelled
            ServerLogger::logInfo("Download cancelled for model: %s", model_id.c_str());
            return true;
        }
    ...
    void DownloadManager::cleanupOldDownloads(int minutes) {
        ...
        try {
            ...
            DownloadResult result = download_file(progress->url, progress->local_path, progressCallback);
