...
namespace kolosal {
    namespace utils {
        ...
        CompletionMetricsResponseModel convertToCompletionMetricsResponse(const AggregatedCompletionMetrics& metrics) {
            ...
            response.summary.success_rate_percent = metrics.totalRequests > 0 ? 
                static_cast<double>(metrics.completedRequests) / metrics.totalRequests * 100.0 : 0.0;
            ...
            for (const auto& engine : metrics.perEngineMetrics) {
                ...
                engineDto.requests.success_rate_percent = engine.totalRequests > 0 ?
                    static_cast<double>(engine.completedRequests) / engine.totalRequests * 100.0 : 0.0;
                ...
                engineDto.tokens.average_input_per_request = engine.completedRequests > 0 ?
                    static_cast<double>(engine.totalInputTokens) / engine.completedRequests : 0.0;
                engineDto.tokens.average_output_per_request = engine.completedRequests > 0 ?
                    static_cast<double>(engine.totalOutputTokens) / engine.completedRequests : 0.0;
                ...
                engineDto.performance.average_turnaround_time_ms = engine.completedRequests > 0 ?
                    engine.totalTurnaroundTime / engine.completedRequests : 0.0;
                engineDto.performance.average_output_generation_time_ms = engine.completedRequests > 0 ?
                    engine.totalOutputGenerationTime / engine.completedRequests : 0.0;
        ...
        EngineMetricsResponseModel convertToEngineMetricsResponse(const CompletionMetrics& metrics) {
            ...
            response.requests.success_rate_percent = metrics.totalRequests > 0 ?
                static_cast<double>(metrics.completedRequests) / metrics.totalRequests * 100.0 : 0.0;
            ...
            response.tokens.average_input_per_request = metrics.completedRequests > 0 ?
                static_cast<double>(metrics.totalInputTokens) / metrics.completedRequests : 0.0;
            response.tokens.average_output_per_request = metrics.completedRequests > 0 ?
                static_cast<double>(metrics.totalOutputTokens) / metrics.completedRequests : 0.0;
            ...
            response.performance.average_turnaround_time_ms = metrics.completedRequests > 0 ?
                metrics.totalTurnaroundTime / metrics.completedRequests : 0.0;
            response.performance.average_output_generation_time_ms = metrics.completedRequests > 0 ?
                metrics.totalOutputGenerationTime / metrics.completedRequests : 0.0;
